...
class UserSessionProxyConnectorIntegrationSpec extends BaseConnectorIntegrationSpec
  ...
  "fetchSession" should {
    "return the session" in new Setup {
      stubFor(
        get(urlPathEqualTo(s"/session/$userSessionId"))
          .willReturn(
            aResponse()
              .withStatus(OK)
      ...
      private val result = await(underTest.fetchSession(userSessionId))

      result.value shouldBe UserSession(userSessionId, loggedInState = LoggedInState.LOGGED_IN, buildTrackedUser(userEmail))
    }
    ...
    "return Fail with session invalid when the session doesnt exist" in new Setup {
      ...
      await(underTest.fetchSession(userSessionId)) shouldBe None
    }
    ...
    "fail on Upstream5xxResponse when the call return a 500" in new Setup {
      ...
      intercept[UpstreamErrorResponse] {
        await(underTest.fetchSession(userSessionId))
  ...
  "deleteSession" should {
    "delete the session" in new Setup {
      stubFor(
        delete(urlPathEqualTo(s"/session/$userSessionId"))
          .willReturn(
            aResponse()
              .withStatus(NO_CONTENT)
          )
      )
      await(underTest.deleteSession(userSessionId)) shouldBe NO_CONTENT
    }
    ...
    "throws for other errors" in new Setup {
      stubFor(
        delete(urlPathEqualTo(s"/session/$userSessionId"))
          .willReturn(
            aResponse()
      ...
      intercept[Throwable] {
        await(underTest.deleteSession(userSessionId))
      }
    }
  }
  ...
  "updateSessionLoggedInState" should {
    ...
    "update session logged in state" in new Setup {
      val url                                                    = s"/session/$userSessionId/loggedInState/LOGGED_IN"
      val session: UserSession                                   = UserSession(userSessionId, LoggedInState.LOGGED_IN, buildTrackedUser())
